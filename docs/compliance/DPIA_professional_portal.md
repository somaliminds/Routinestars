...
(§F2 defines the per-role view). Default is the *least* data; the parent widens
scope explicitly.
...
| Art.9 condition (special category) | **Explicit consent (Art.9(2)(a))** captured from the parent. Where a professional processes it under their own health/social-care duty, Art.9(2)(h) applies to *them*, not to RoutineStars' sharing. |
...
| Professional account compromised | Low | High | Professional auth with MFA (see §7 open decision); short-lived sessions |
...
## 7. Open product/architecture decisions (block build until resolved)

1. **How do professionals authenticate?** Options: (a) separate professional
   accounts via Supabase Auth with MFA; (b) magic-link, time-boxed access; (c)
   extend the existing care-team invite. Recommendation: (a) with MFA — highest
   assurance for special-category data.
2. **Is a professional identity verified** (e.g. HCPC/GMC registration number)?
   For MVP, capture role + org + reg number self-declared; verify manually for
   paid Professional-tier accounts.
3. **Contribution scope** — confirm professionals can only add advice/targets,
   never edit the child's core records.
4. **Where does the DPIA sign-off sit** — RoutineStars Ltd director as data
   controller; consider ICO consultation if residual high risk remains.

---

## 8. Conclusion

The professional portal is viable and strategically strong, but it is
**governance-first**: the consent ledger (§4.1), the audit log (§4.2), the
Children's-Code default-privacy posture (§5), and a decision on professional
authentication (§7) must be in place **before** any child's data is shared with
a professional. This DPIA must be approved and its mitigations implemented as
the first build increment of Phase B — not retrofitted after the UI.

**Next step:** approve this DPIA, resolve the §7 open decisions, then build in
order: consent ledger → audit log → professional auth → role-scoped views →
